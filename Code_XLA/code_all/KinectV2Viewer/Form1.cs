...
using static KinectV2Viewer.GlobalSatics;
using Alturos.Yolo;
...
namespace KinectV2Viewer
{
    ...
    {
        SpeechRecognitionEngine recEngine = new SpeechRecognitionEngine(); //Test 
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            commands.Add(new String[] { "pick apple", "pick coca" });
        ...
        void recEngine_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            switch (e.Result.Text)
            {
                case "pick apple":
                    ...
                    MessageBox.Show("Đã gắp được quả táo");
                    break;
                case "pick coca":
                    {

                    }
                    MessageBox.Show("Đã gắp được coca");
                    break;
            }
        }
        //End test
        private void UpdateKinect()
        {
            ...
            depthWidth = mySensor.DepthFrameSource.FrameDescription.Width;
            depthHeight = mySensor.DepthFrameSource.FrameDescription.Height;
            ...
            for (int i = 0; i < cameraSpacePoints.Length; i++)
            {
                ...
                else if (p.Z <= depthLimit)
                {
                    ...
                    else if (System.Single.IsNegativeInfinity(p.X) == false)
                    {
                        ...
                        {
                            int colorIndex = ((colorY * colorWidth) + colorX) * 32 / 8;
                        ...
                        color_pc_idx.Add(colorX);//test
                        color_pc_idx.Add(colorY);//
                        color_pc_idx.Add(ptIdx); //
                    }
                }
            }
        ...
        private void buttonOpenPly_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog.InitialDirectory = "C:\\Users\\DELL PRECISION\\Desktop\\Code_XLA\\Image";
            tabControl1.SelectedIndex = 1;
        ...
        private void buttonTestPlaneDetection_Click(object sender, EventArgs e)
        {
            ...
            double[] o = new double[3];              // tạo độ của gốc
        ...
        private void buttonSavePLY_Click(object sender, EventArgs e)
        {
            ...
            saveFileDialog1.InitialDirectory = "C:\\Users\\DELL PRECISION\\Desktop\\Code_XLA\\Image";
        ...
        // Start Edit error
        delegate void deleDataReceive();
        public void DataReceived()
        {
            if (listBoxReceived.InvokeRequired)
            {
                this.Invoke(new deleDataReceive(DataReceived));
            }
            else
            {
                if (serialPort1.IsOpen)
        ...
        {
            // Save Image
            string rgb_path = @"C:\Users\DELL PRECISION\Desktop\Code_XLA\Image\capture_" + idxSaveImage.ToString() + ".jpg";
            pictureBoxColor.Image.Save(rgb_path, System.Drawing.Imaging.ImageFormat.Jpeg);
            idxSaveImage++;
            /* Xuất ra datagridviewfile
             * var imageInfos = new DirectoryImageReader().Analyze(@" đường dẫn");
             * this.dataGridView.DataSource = Image.Infos.Tolist();
        ...
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog() { Filter = "PNG|*.png|JPEG|*.jpeg|JPG|*.jpg|BMP|*.bmp" })
            {
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
        ...
        private void btnDetect_Click(object sender, EventArgs e)
        {
            ...
            using (var yoloWrapper = new YoloWrapper(GG.YOLOv3.ConfigFile, GG.YOLOv3.WeightsFile, GG.YOLOv3.NamesFile))
            {
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    AddDetails(pictureBox1, items); //đóng khung items, more infomation
                }
        ...
        //start test 02/6;
        Bitmap image1;
        private void btnDetectObj_Click(object sender, EventArgs e)
        {
            CheckForIllegalCrossThreadCalls = false;
            // Save Image
            string rgb_path = @"C:\Users\DELL PRECISION\Desktop\Code_XLA\Image1\capture1" + ".bmp";
            
            pictureBoxColor.Image.Save(rgb_path, System.Drawing.Imaging.ImageFormat.Jpeg);
            // Retrieve the image.
            image1 = new Bitmap(@"C:\Users\DELL PRECISION\Desktop\Code_XLA\Image1\capture1.bmp", true);
            ...
            using (var yoloWrapper = new YoloWrapper(GG.YOLOv3.ConfigFile, GG.YOLOv3.WeightsFile, GG.YOLOv3.NamesFile))
           {
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    AddDetails1(pictureBox1, items);
                }
            }
        }
        void AddDetails1(PictureBox pictureBox, List<YoloItem> items)
        {
            getPointsCloud();
            int colorX = color_pc_idx[0];
            int colorY = color_pc_idx[1];
            int pcIdx = color_pc_idx[2];
            vtkPoints output = vtkPoints.New();
            ...
            foreach (var item in items)
            {
                for (int i = 0; i < cameraSpacePoints.Length; i++)
                {
                    ...
                    if ((colorX >= x) && (colorX <= (x+colorWidth)) && (colorY >= y) && (colorY <= (y+colorHeight)))
                        {
                        int colorIndex = ((colorY * colorWidth) + colorX) * 32 / 8;
                        Byte b = 0; Byte g = 0; Byte r = 0;
            ...
            /* Renderer.RemoveAllViewProps();
            poly2.SetPoints(output);
            points = output;
            scenePolyData.ShallowCopy(poly2);
            vtkVertexGlyphFilter gly1 = vtkVertexGlyphFilter.New();
            gly1.SetInput(poly2);
            gly1.Update();
            mapper.SetInputConnection(gly1.GetOutputPort());
